...
별도의 서버 구축이나 로그인, 설치 과정 없이 웹 브라우저에서 즉시 C++, Python, Java 코드를 작성하고, 다수의 테스트 케이스를 통해 로직을 검증하며, 결과를 URL로 손쉽게 공유할 수 있는 정적 웹 애플리케이션
...
지원 언어는 **단일 진실 공급원** 방식으로 관리됩니다. 새 언어를 추가할 때는 다음만 수정하면 됩니다 (드롭다운·저장소·복원은 자동 반영).

| 단계 | 파일 | 작업 |
|------|------|------|
| 1 | `api.js` | `LANGUAGE_CONFIG`에 `{ language, version, displayName }` 추가 |
| 2 | `app.js` | CodeMirror `import` 추가 후 `LANG_EXTENSIONS`에 한 줄, `templates`에 템플릿 코드 추가 |
| 3 | `index.html` | import map에 `@codemirror/lang-xxx` URL 한 줄 추가 |
