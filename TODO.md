...
- [x] 화면 크기 조절하여 가로 스크롤바 생기지 않게 하기
  - [x] `html, body`에 `overflow-x: hidden`, `max-width: 100vw` 적용
  - [x] `.container-fluid`에 `max-width: 100%` 및 좌우 패딩 0 적용
- [x] 언어 선택 버튼을 공유 버튼 바로 왼쪽으로 옮기기
  - [x] 중앙 배치 제거하고 우측 영역으로 이동
  - [x] 언어 선택 드롭다운과 공유 버튼을 `gap-2`로 간격 설정하여 배치
- [x] 공유 버튼 오른쪽에 내 깃허브 프로필 링크(https://github.com/dapin1490) 추가하기: Bootstrap Icons의 `bi-github` 아이콘 사용
  - [x] Bootstrap Icons CDN 추가
  - [x] GitHub 링크 버튼 추가 (`bi-github` 아이콘 사용)
  - [x] 새 탭에서 열리도록 `target="_blank"` 및 `rel="noopener noreferrer"` 설정
- [x] 공유 버튼 텍스트를 "공유"로 바꾸기
  - [x] `index.html`의 공유 버튼 텍스트를 "Share"에서 "공유"로 변경
- [x] 공유 버튼을 텍스트 없는 아이콘으로 변경
  - [x] Bootstrap Icons의 `bi-share-fill` 아이콘 사용
  - [x] 클릭 시 2초 간 bi-check-lg 아이콘 표시하여 피드백
  - [x] 정상 공유 시 토스트 메시지 노출되지 않음
    - [x] 정상 공유 성공 시 아이콘만 변경하여 피드백 제공
    - [x] 오류 발생 시에만 토스트 메시지 표시
- [x] 화면 좌우에 여백 추가: 약간의 여백을 추가하여 헤더의 제목과 버튼이 화면 양 끝에 붙지 않도록 하기
  - [x] 네비게이션 바의 `container-fluid`에 좌우 패딩 1rem 추가
  - [x] 메인 컨테이너는 패딩 0 유지하여 가로 스크롤바 방지
