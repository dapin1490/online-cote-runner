...
- [ ] 순차 실행 로직 구현
  - [ ] `for...of` 또는 `for` 루프를 사용한 순차 실행
  - [ ] 각 테스트 케이스를 하나씩 실행하고 결과를 수집
  - [ ] 각 케이스 실행 완료 후 다음 케이스 실행
- [ ] 실시간 결과 표시 구현
  - [ ] 각 케이스 실행 완료 시마다 UI 업데이트
  - [ ] 진행 중인 케이스와 완료된 케이스를 구분하여 표시
  - [ ] 진행 중인 케이스에 로딩 스피너 표시
  - [ ] 완료된 케이스는 즉시 결과 표시 (Pass/Fail/Error)
- [ ] 요청 실패 시에도 나머지 결과 처리 보장
  - [ ] try-catch로 각 케이스 실행 에러 처리
  - [ ] 에러 발생 시에도 다음 케이스 계속 실행
  - [ ] 에러 결과를 결과 배열에 포함
